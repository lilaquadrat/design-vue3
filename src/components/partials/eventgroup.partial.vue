...
<template>
  <section class="lila-eventgroup-partial">
    ...
    <section class="events-container">
      ...
      <section class="single-event" v-for="(single, index) in useElements" :class="{ hasPicture: single.picture }"
        :key="`events-${index}`">
        ...
        <section class="event-info">
          ...
          <section class="date-location">
            <section class="date">
              <div class="day">
                <template v-if="!single.dateString">{{ $helpers.date(single.start) }} - {{ $helpers.date(single.end) }}
                </template>
                {{ single.dateString }}
              </div>
            </section>
            <div class="location"><lila-icons-partial type="location" size="large" class="icon green" /> {{
              single.location }}</div>
          </section>
