...
const isLeft = computed((): boolean => props.variant?.includes('left'));
...
const calculateOptionsStyle = (attachTo: HTMLElement) => {
...
<template>
    <nav ref="element" :id="id" :class="[inviewState, variant, { open: open }]" class="lila-menu-module lila-module">
            ...
            <section class="links" v-if="!isLeft">
              <template v-for="(element, index) in elementsArray">
                ...
                <section :key="`group-${index}`" v-if="element.links" class="link-group main">
                  ...
                  <lila-overlay-background-partial v-if="element.links && element.active" background="none">
                      <ul class="link-list" >
